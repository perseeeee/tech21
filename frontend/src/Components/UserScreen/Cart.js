...
const ensureCartTable = async (db) => {
  ...
  const tableInfo = await db.getAllAsync('PRAGMA table_info(cart_items);');
  const columns = tableInfo.map(col => col.name);

  if (!columns.includes('discountedPrice')) {
    await db.runAsync('ALTER TABLE cart_items ADD COLUMN discountedPrice REAL;');
  }

  if (!columns.includes('discountPercentage')) {
    await db.runAsync('ALTER TABLE cart_items ADD COLUMN discountPercentage REAL;');
  }

  if (!columns.includes('isOnSale')) {
    await db.runAsync('ALTER TABLE cart_items ADD COLUMN isOnSale INTEGER DEFAULT 0;');
  }
};

const getDB = async () => {
  if (_db && !_dbInitPromise) return _db;
  if (_dbInitPromise) {
  ...
  _dbInitPromise = (async () => {
    _db = await SQLite.openDatabaseAsync('pelagos.db');
    await ensureCartTable(_db);
  })();

  try {
    await _dbInitPromise;
    console.log('SQLite ready: cart_items table ensured');
  } catch (err) {
    _db = null;
      ...
      <View style={styles.itemInfo}>
        ...
        <View style={styles.qtyRow}>
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <Icon name="remove" size={16} color="#0EA5E9" />
          </TouchableOpacity>
          ...
          {cartItems.length > 0 && (
            <View style={styles.topBarActions}>
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Icon name="delete-sweep" size={20} color="#0EA5E9" />
...
const styles = StyleSheet.create({
  ...
  centered: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#F3F8FC' },
  ...
  topBarSubtitle: { fontSize: 13, color: '#64748B', marginTop: 2 },
  topBarActions: {
    flexDirection: 'row',
    alignItems: 'center',
  },
  ...
  clearBtn: {
    ...
    backgroundColor: '#F7FBFF', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20, borderWidth: 1, borderColor: '#BAE6FD',
  },
  clearBtnText: { fontSize: 13, fontWeight: '600', color: '#0EA5E9', marginLeft: 5 },
  ...
  shopNowBtn: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: '#0EA5E9', paddingHorizontal: 24, paddingVertical: 13,
  ...
  itemImageBox: {
    ...
    overflow: 'hidden', backgroundColor: '#F3F8FC', marginRight: 12,
  },
  ...
  carouselDotActive: { backgroundColor: '#0EA5E9', width: 6, height: 6, borderRadius: 3 },
  itemInfo: { flex: 1 },
  ...
  itemPrice: { fontSize: 15, fontWeight: '800', color: '#0EA5E9' },
  discountedPrice: {
    color: '#0EA5E9',
  },
  originalPrice: {
  ...
  qtyBtn: {
    ...
    backgroundColor: '#F7FBFF', borderWidth: 1, borderColor: '#BAE6FD',
    justifyContent: 'center', alignItems: 'center',
  },
  qtyBtnDisabled: { opacity: 0.4 },
  qtyText: { fontSize: 15, fontWeight: '700', color: '#0F172A', minWidth: 26, textAlign: 'center' },
  subtotalText: { fontSize: 12, color: '#64748B', marginLeft: 6 },
  ...
  summaryTotalLabel: { fontSize: 16, fontWeight: '700', color: '#0F172A' },
  summaryTotalValue: { fontSize: 22, fontWeight: '900', color: '#0EA5E9' },
  checkoutBar: {
  ...
  checkoutBtn: {
    ...
    backgroundColor: '#0EA5E9', paddingVertical: 15, borderRadius: 14, gap: 8,
    shadowColor: '#0C4A6E', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.16, shadowRadius: 8, elevation: 3,
  },
